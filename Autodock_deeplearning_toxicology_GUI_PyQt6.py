...
from PyQt6.QtCore import QObject, pyqtSignal, pyqtSlot, QThread, QThreadPool, QRunnable, QUrl, QSize, QRect, QFileInfo, QTimer
from PyQt6 import QtWidgets 
from PyQt6.QtWidgets import *#QMainWindow, QApplication, QWidget, QVBoxLayout, QGridLayout, QGroupBox, QPushButton, QLabel, QTableWidget, QTableWidgetItem, QListWidget, QListWidgetItem, QLineEdit, QInputDialog, QMessageBox, QFileDialog, QDockWidget, QScrollArea, QFormLayout, QStatusBar, QTreeView
...
class check_progres(QThread):
    ...
    print("check progress at the top under QThread where is first appears")
    
    def __init__(self, **kwargs):
        super().__init__()  # Properly initialize as QThread
        self.kwargs = kwargs
                    ...
                    try:
                       ...
                       lookup = pcp.download('SDF', name +"_pubchem-version"+'.sdf', cid, record_type='3d')
                        ...
                        try:
                            ...
                            print(f"Error downloading 3D structure for {name}: {str(e)}")
                            pass
                    else:
                       pass
                elif cid != [0] and len(cid) > 1:
                    for i in cid:
                         print("There is more than one CID number this is one: ", name)
...
class process_results(QObject):
    ...
    def run_models(self, dframe, dlgpath, model_to_load, model_type_match, results_files):
        IMG_HEIGHT = 210
        IMG_WIDTH = 210
        prediction_image_generator = ImageDataGenerator(rescale=1./255)
        pred_data_gen = prediction_image_generator.flow_from_dataframe(
            dataframe=dframe,
            directory=dlgpath,
        ...
        #total_steps = len(dframe)
        #predictionstenflo = model_X.predict(pred_data_gen, batch_size=1, verbose=1, steps=total_steps)
        # Get total number of samples for tracking completion
        total_samples = len(dframe)
        samples_processed = 0
        ...
        # Manually process batches and track completion
        predictionstenflo = []
        
        # Create a completion flag
        prediction_complete = False
        
        print(f"Starting prediction of {total_samples} samples...")
        
        # Process each batch manually to track progress
        while not prediction_complete:
            try:
                # Get the next batch (should be only 1 image with batch_size=1)
                batch = next(pred_data_gen)
                
                # Process this batch
                batch_prediction = model_X.predict(batch, verbose=0)
                
                # Append to our results
                if samples_processed == 0:
                    predictionstenflo = batch_prediction
                else:
                    predictionstenflo = np.vstack((predictionstenflo, batch_prediction))
                ...
                prediction_complete = True
                print("StopIteration reached - all samples processed!")
        ...
        tf.keras.backend.clear_session()
        print(f"This is what results_files looks like: {results_files}")
        return
      ...
      ## Right NOW 2-4-2021 this has to be in results_files and I want it in install_dir
       ...
       for files in os.listdir():
          if files.startswith("parsed_"):
             ...
             print(f"this is the model_to_load: model_to_load and model_type_match: {model_to_load} and {model_type_match}")
             self.run_models(dframe, dlgpath, model_to_load, model_type_match, results_files)
          else:
             continue
       return
...
class MainWindow(QMainWindow):
    ...
    print("this is the end of the main window")
    ...
    def initUI(self):
        ...
        self.addDockWidget(Qt.DockWidgetArea.LeftDockWidgetArea, dockWidget_setupdocking)
        #self.addDockWidget(Qt.TopDockWidgetArea, dockWidget_dockingresults)
        self.addDockWidget(Qt.DockWidgetArea.TopDockWidgetArea, dockWidget_dockingresults)
        ...
        add_existing_docking.triggered.connect(self.open_existing_project)
        file_menu.addAction(add_existing_docking)
    ...
    def molfile_start(self):
           #PYQT6 has a weird problem where if you leave the last cell active
           #it won't read it. So I have to force it to commit the data
           #PyQt6 addtion that wasn't necessary in PyQt5
           # First, commit any active cell edits
           current_editor = QApplication.focusWidget()
           if isinstance(current_editor, QLineEdit):  # Check if it's a cell editor
               # Force the editor to commit its data
    ...
    def setup_docking_project(self):
        ...
        #PyQt5 code change for pyqt6 below
        #text_input, okPressed = QInputDialog.getText(self, "Project Name","Enter a New Project Name:", QLineEdit.Normal, "")
        text_input, okPressed = QInputDialog.getText(self, "Project Name","Enter a New Project Name:")
    ...
    def open_existing_project(self):
        ...
        try:
            ...
            install_dir = os.getcwd()
            print("this is install_dir from open_existing_project: ", install_dir)
        ## This will open a dialog box to get the name of a new project and then a directory will be made with that name with all project directoies
        filename_expro = QFileDialog.getExistingDirectory(self,"Existing Project")
        #home = os.getcwd()
        print("this is filename_expro:", filename_expro)
        ## this will provide information of current directories to check for re-used names later
        #check_existing = os.listdir(home)
        if filename_expro != '':
            ## now we check for existing directories
        ...
        ##PyQt6 changes
        if self.formLayout.itemAt(1) != None:
        ...
        self.tree.setRootIndex(self.model.index(working_project))
        return working_project
    print("this is the end of the main window3")
    ...
    def process_results(self):
        try:
            ...
            def summarize_and_plot():
                ...
                return
            def run_model():
                self.threadmodel = QThreadPool()
                self.start_log = process_results()
                
                # Add flag to track if summarize_and_plot has been triggered
                self.next_step_triggered = False
                # Create a completion verification flag and lock for thread safety
                self.tensorflow_ops_complete = False
                ...
                def interact_diagrams(install_dir, results_files, dlg_files):
                    ...
                    # Add explicit sync point: force a small TensorFlow operation to complete
                    # This ensures all previous operations have finished
                    tf.keras.backend.clear_session()
                    dummy = tf.constant([1.0])
                    result = tf.reduce_sum(dummy).numpy()
                    # Set completion flag only at the very end of the function
                    self.tensorflow_ops_complete = True
                    # Print confirmation of completion
                    print("TensorFlow operations complete flag set")
                
                # Modify update_status to check for 100% and trigger the next step
                original_update_status = self.update_status
                
                def progress_based_trigger(prog):
                    # Call the original update function
                    original_update_status(prog)
                    print(f"Progress: {prog}%")
                    # Check if we've reached 100% and haven't triggered the next step yet
                    if prog == 100 and not self.next_step_triggered:
                        self.next_step_triggered = True
                        print("Progress reached 100%, starting summarize_and_plot")
                        QTimer.singleShot(500, summarize_and_plot)
                        
                # Replace the update_status method temporarily
                self.update_status = progress_based_trigger
                ...
                # Connect the finished signal to thread_complete
                worker_process2.signals.finished.connect(self.thread_complete)
                
                # When worker is done, run a verification timer that checks completion status
                def verify_tensorflow_completion():
                    if self.tensorflow_ops_complete:
                        print("Verified: TensorFlow operations have completed")
                        # If progress is already 100%, manually trigger plot
                        current_progress = self.progressBar.value()
                        if current_progress == 100 and not self.next_step_triggered:
                            self.next_step_triggered = True
                            print("Progress already at 100%, manually triggering summarize_and_plot")
                            QTimer.singleShot(1000, summarize_and_plot)
                    else:
                        print("TensorFlow operations still running, checking again in 2 seconds...")
                        QTimer.singleShot(2000, verify_tensorflow_completion)
                
                # Start the worker in the thread pool
                self.threadmodel.start(worker_process2)
                
                # When worker signals finished, start verification timer
                worker_process2.signals.finished.connect(
                    lambda: QTimer.singleShot(1000, verify_tensorflow_completion))
                
                # Clean up method to restore original update_status 
                def cleanup():
                    self.update_status = original_update_status
                    self.next_step_triggered = False
                
                # Connect cleanup to happen after all processing is complete
                # but don't clear the thread pool
                worker_process2.signals.finished.connect(cleanup)
    ...
    def addcolumn(self):
        text, okPressed = QInputDialog.getText(self, "Get text","Text:")
    ...
    def createtopRightGroupbox3(self, file_heatmap_mass):
        ...
        print("this is file_heatmap_mass: ", file_heatmap_mass)
...
if __name__ == '__main__':
    #import os
    # Disable Qt's thread priority adjustments
    #os.environ["QT_THREAD_PRIORITY_SCALE"] = "0"
    
    #os.environ["QTWEBENGINE_CHROMIUM_FLAGS"] = "--no-sandbox --disable-gpu"
    #os.environ["QTWEBENGINE_DISABLE_SANDBOX"] = "1"
    #print("trying step one")
    try:       
        # Import QApplication here to ensure environment vars are set first
        ...
        # Disable thread priority changes at the Qt level
